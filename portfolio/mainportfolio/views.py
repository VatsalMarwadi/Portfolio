...
def _handle_contact_post(request, active_section):
    ...
    email_result = send_inquiry_emails(request, inquiry)
    ...
    if _is_ajax(request):
        ...
        if not email_result.get("skipped") and not (
            email_result.get("user") and email_result.get("admin")
        ):
            payload["message"] = (
                "Your message was saved. Email delivery may be delayed — "
                "we will still get back to you."
            )
        return JsonResponse(payload)

    if email_result.get("skipped") or not email_result.get("admin"):
        messages.warning(
            request,
            "Your message was saved. If you do not receive a confirmation email, "
            "we may still reply to you directly.",
        )
    else:
